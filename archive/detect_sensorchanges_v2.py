...
for event in detect.event_gen():
	if event is not None:
		...
		if(data_set_count_node_1 >= data_set_count):

			#Reset number of data sets for node 1
			data_set_count_node_1 = 0
		...
		if(data_set_count_node_2 >= data_set_count):
			# Reset number of data sets for node 1 & 2
			data_set_count_node_2 = 0
			...
			CALC_wind_delta_node_2 = float(data_sets_node_2[-data_set_count][1])-float(data_sets_node_2[-1][1])
			...
			if(CALC_ABS_tmp_delta_node_2 >= tmp_delta or CALC_ABS_wind_delta_node_2 >= wind_delta):
				...
				if(CALC_ABS_wind_delta_node_2 >= wind_delta):
					if(event_detected_node_2_count_wind == 0):
						...
						if(time_to_epoch(data_sets_node_2[-1][0])-time_to_epoch(str(event_detected_node_2_ts_tmp))> detection_timeout_wind):
